...
final class SpanTranslator {
	...
	// https://github.com/open-telemetry/opentelemetry-java/blob/3e8092d086967fa24a0559044651781403033313/api/all/src/main/java/io/opentelemetry/api/trace/TraceId.java#L32
	static final ByteString INVALID_TRACE_ID = ByteString.fromHex("00000000000000000000000000000000");

	// Same value as the API
	// https://github.com/open-telemetry/opentelemetry-java/blob/3e8092d086967fa24a0559044651781403033313/api/all/src/main/java/io/opentelemetry/api/trace/SpanId.java#L28
	static final ByteString INVALID_SPAN_ID = ByteString.fromHex("0000000000000000");

	// Required to be set to non-empty string.
	// https://github.com/open-telemetry/opentelemetry-proto/blob/14afbd4e133ee8a8a5a9f7a0fd3a09d5a9456340/opentelemetry/proto/trace/v1/trace.proto#L142-L143
	...
	// Same default value as the SDK
	// https://github.com/open-telemetry/opentelemetry-java/blob/3e8092d086967fa24a0559044651781403033313/sdk/common/src/main/java/io/opentelemetry/sdk/resources/Resource.java#L46-L52
	static final String DEFAULT_SERVICE_NAME = "unknown_service:java";

	// Defined value in
	// https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry/#telemetry-sdk-language
	...
	TracesData translate(MutableSpan braveSpan) {
		...
		io.opentelemetry.proto.common.v1.InstrumentationScope.Builder scopeBuilder = io.opentelemetry.proto.common.v1.InstrumentationScope
			.newBuilder();
	...
	private Span.Builder builderForSingleSpan(MutableSpan span, ResourceSpans.Builder resourceSpansBuilder) {
		...
		if (!this.resourceAttributes.containsKey(SemanticConventionsAttributes.SERVICE_NAME)) {
			...
			resourceBuilder
				.addAttributes(stringAttribute(SemanticConventionsAttributes.SERVICE_NAME, localServiceName));
		}
		resourceAttributes.forEach((k, v) -> resourceBuilder.addAttributes(stringAttribute(k, v)));
		// Set Telemetry SDK resource attributes
		// https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry/
		resourceBuilder
			.addAttributes(
					stringAttribute(SemanticConventionsAttributes.TELEMETRY_SDK_LANGUAGE, TELEMETRY_SDK_LANGUAGE))
	...
	static final class AnnotationMapper implements AnnotationConsumer<Span.Builder> {
		...
		public void accept(Span.Builder target, long timestamp, String value) {
			target.addEvents(Event.newBuilder()
				.setTimeUnixNano(TimeUnit.MICROSECONDS.toNanos(timestamp))
				.setName(value)
				.build());
		}
