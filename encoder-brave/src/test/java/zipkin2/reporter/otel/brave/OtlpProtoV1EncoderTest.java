...
class OtlpProtoV1EncoderTest {
	...
	void defaultConfiguration() {
		OtlpProtoV1Encoder encoder = OtlpProtoV1Encoder.newBuilder().build();
	...
	void customInstrumentationScope() {
		...
		OtlpProtoV1Encoder encoder = OtlpProtoV1Encoder.newBuilder().instrumentationScope(instrumentationScope).build();
		SpanTranslator spanTranslator = encoder.spanTranslator;
	...
	void customErrorTag() {
		...
		OtlpProtoV1Encoder encoder = OtlpProtoV1Encoder.newBuilder().errorTag(errorTag).build();
		SpanTranslator spanTranslator = encoder.spanTranslator;
		assertThat(spanTranslator.instrumentationScope).isEqualTo(BraveScope.instrumentationScope());
		assertThat(spanTranslator.resourceAttributes).isEqualTo(Collections.emptyMap());
	...
	void customTagToAttributes() {
		TagToAttributes tagToAttributes = TagToAttributes.newBuilder().tagToAttribute("abc", "xyz").build();
		OtlpProtoV1Encoder encoder = OtlpProtoV1Encoder.newBuilder().tagToAttributes(tagToAttributes).build();
		SpanTranslator spanTranslator = encoder.spanTranslator;
