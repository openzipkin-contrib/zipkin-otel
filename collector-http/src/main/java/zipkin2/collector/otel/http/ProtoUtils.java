...
final class ProtoUtils {
	...
	static String valueToString(AnyValue value) {
		...
		if (value.hasArrayValue()) {
			...
			// the Otel Zipkin Exporter doesn't implement like that
			// https://github.com/open-telemetry/opentelemetry-java/blob/main/exporters/zipkin/src/test/java/io/opentelemetry/exporter/zipkin/OtelToZipkinSpanTransformerTest.java#L382-L385
			// Also Brave doesn't use the json encoding.
			// So follow the comma separator here.
			return value.getArrayValue().getValuesList().stream().map(ProtoUtils::valueToString).collect(joining(","));
		}
