...
class ZipkinTestUtil {
	...
	private static final List<EventData> annotations = Collections
		.unmodifiableList(Arrays.asList(EventData.create(1505855799_433901068L, "RECEIVED", Attributes.empty()),
	...
	static ExportTraceServiceRequest.Builder requestBuilder(
		...
		return ExportTraceServiceRequest.newBuilder()
			.addResourceSpans(ResourceSpans.newBuilder()
				.setResource(resourceCustomizer
					.apply(Resource.newBuilder().addAttributes(stringAttribute("service.name", "tweetiebird"))))
				.addScopeSpans(ScopeSpans.newBuilder()
					...
					.addSpans(spanCustomizer.apply(io.opentelemetry.proto.trace.v1.Span.newBuilder()
						...
						.addEvents(Event.newBuilder().setName("RECEIVED").setTimeUnixNano(1505855799_433901068L))
						.addEvents(Event.newBuilder().setName("SENT").setTimeUnixNano(1505855799_459486280L))))));
	}
	...
	static KeyValue attribute(String key, Function<AnyValue.Builder, AnyValue.Builder> builder) {
		return KeyValue.newBuilder().setKey(key).setValue(builder.apply(AnyValue.newBuilder())).build();
	}
