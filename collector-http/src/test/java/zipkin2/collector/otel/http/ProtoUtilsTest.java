...
class ProtoUtilsTest {
  ...
  void testValueToJson() {
    assertThat(valueToJson(AnyValue.newBuilder().setStringValue("string").build())).isEqualTo(
        "\"string\"");
    ...
    assertThat(valueToJson(AnyValue.newBuilder().setKvlistValue(KeyValueList.newBuilder()
        .addValues(
            KeyValue.newBuilder().setKey("x").setValue(AnyValue.newBuilder().setStringValue("abc")))
        .addValues(
            KeyValue.newBuilder().setKey("y").setValue(AnyValue.newBuilder().setStringValue("efg")))
        .addValues(KeyValue.newBuilder().setKey("z").setValue(AnyValue.newBuilder().setIntValue(0)))
        .build()).build()))
        .isEqualTo("{\"x\":\"abc\",\"y\":\"efg\",\"z\":0}");
    assertThat(
        valueToJson(AnyValue.newBuilder().setBytesValue(ByteString.fromHex("cafebabe")).build()))
  ...
  void testKvListToJson() {
    assertThat(kvListToJson(Arrays.asList(KeyValue.newBuilder()
            .setKey("string")
            .setValue(AnyValue.newBuilder().setStringValue("s"))
            .build(),
        KeyValue.newBuilder()
            .setKey("int")
        ...
        KeyValue.newBuilder()
            .setKey("kvlist")
            .setValue(AnyValue.newBuilder().setKvlistValue(KeyValueList.newBuilder()
                .addValues(KeyValue.newBuilder()
                    .setKey("x")
                    .setValue(AnyValue.newBuilder().setStringValue("abc")))
                .addValues(KeyValue.newBuilder()
                    .setKey("y")
                    .setValue(AnyValue.newBuilder().setStringValue("efg")))
                .addValues(KeyValue.newBuilder()
                    .setKey("z")
                    .setValue(AnyValue.newBuilder().setIntValue(0)))
                .build()))
            .build())))
        .isEqualTo(
            "{\"string\":\"s\",\"int\":100,\"boolean\":true,\"double\":1.2,\"array\":[\"abc\",20,false],\"kvlist\":{\"x\":\"abc\",\"y\":\"efg\",\"z\":0}}");
  }
}
