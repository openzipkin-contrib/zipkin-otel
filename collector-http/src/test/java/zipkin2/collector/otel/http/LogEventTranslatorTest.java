...
class LogEventTranslatorTest {
	...
	void nullSpanShouldBeReturnedWithoutSpanId() {
		Span span = logEventTranslator.generateSpan(
				LogRecord.newBuilder().setSpanId(ByteString.fromHex("7180c278b62e8f6a216a2aea45d08fc9")).build());
	...
	void nullSpanShouldBeReturnedWithoutTraceId() {
		Span span = logEventTranslator.generateSpan(
				LogRecord.newBuilder().setTraceId(ByteString.fromHex("6b221d5bc9e6496c6b221d5bc9e6496c")).build());
		assertThat(span).isNull();
	}
